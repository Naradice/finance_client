...
class CSVClientBase(ClientBase, metaclass=ABCMeta):
    ...
    def _initialize_file_name_func(self, file_names: list):
        ...
        if len(same_strs) > 0:
            if len(same_strs) > 1:
                if len(same_strs) > 2:
                    logger.warning("filenames have a few same words. symbol name may be created unexpectedly.")
                prefix = same_strs[0]
                suffix = same_strs[-1]
                self._get_symbol_from_filename = lambda file_name: os.path.abspath(file_name).replace(prefix, "").replace(suffix, "")
                logger.debug(f"symbol name for column name is automatically generated by removing prefix {prefix} and suffix {suffix}")
                if self.file_name_generator is None:
                    self.file_name_generator = lambda symbol: f"{prefix}{symbol}{suffix}"
                else:
                    logger.debug("file_name_generator is not initialized automatically as it is specified manually.")
            else:
                suffix = same_strs[-1]
    ...
    def _initialize_date_index(self, data: pd.DataFrame, ascending: bool):
        ...
        if self.frame is None:
            ...
            if self.frame != frame:
                logger.warning("different frame data is provided. it break some features. use index frame instead")
                self.frame = frame
        ...
        if data is not None and len(data) > 0:
            if start_date is not None and isinstance(start_date, datetime.datetime):
                ...
                if remaining_length > 0:
                    ...
                    is_date_found = True
                    logger.debug(f"step index is set to {self._step_index}. start date is {data.index[start_index]}")
                else:
                    self._step_index = len(data.index)
                    logger.warning(f"start date {start_date} doesn't exit in the index")
    ...
    def _create_dfs_by_files(self, files, symbols, kwargs):
        ...
        for index in range(0, len(files)):
            ...
            try:
                ...
                logger.error(f"file, {file}, is handled by other process {e}")
                raise e
            except Exception as e:
                logger.error(f"error occured on read csv {e}")
                raise e
    ...
    def _create_datetime_index(self, start_date, frame, data_length):
        if start_date is not None and frame is not None and data_length is not None:
            ...
            logger.warning("any of parameters are None on create_datetime_index")
    ...
    def __init__(
        ...
        if slip_type in self.available_slip_type:
            ...
            logger.warning(f"{slip_type} is not in availble values: {self.available_slip_type}")
            logger.info("use random as slip_type")
            self._get_current_bid = lambda open_value, low_value: random.uniform(low_value, open_value)
            self._get_current_ask = lambda open_value, high_value: random.uniform(open_value, high_value)
        ...
        if self._step_index > len(self):
            logger.warning(f"step index {self._step_index} is greater than data length {len(self)}")
    ...
    def _read_csv(self, files, symbols=None, columns=None, date_col=None, skiprows=None, start_date=None, frame=None):
                ...
                def convert_datetime(df, mixed=None):
                    try:
                        ...
                        if mixed is None:
                            ...
                            logger.exception("failed to convert index to datetime.")
                            return df, False
        ...
        if is_date_index is False:
            if start_date is not None and frame is not None:
                ...
                logger.error("Couldn't daterming date column")
    ...
    def __get_rates(self, index, length, symbols, frame, columns):
        if length is None or length == slice(None):
            ...
            try:
                ...
                logger.error(f"can't find columns in data: {e}")

            try:
                rates = rates.iloc[index - length : index]
            except Exception as e:
                logger.error(f"can't find data fom {index - length} to {index}: {e}")

            if frame is not None and self.frame < frame:
                rates = self.roll_ohlc_data(rates, frame, grouped_by_symbol=True)
                rates.dropna(thresh=len(rates.columns), inplace=True)
    ...
    def _get_ohlc_from_client(self, length=None, symbols: list = None, frame: int = None, columns=None, index=None, grouped_by_symbol: bool = False):
        ...
        if len(missing_data) > 0:
            try:
                ...
                logger.exception("Filed to concat existing data with additional data of specified symbols")

        target_columns = None
        if len(target_symbols) == 0:
            logger.warning(f"Specified symbols can't be handled as csv file or its symbol name: {symbols}")
            return pd.DataFrame()
        ...
        if length is not None:
            if index < length - 1:
                logger.warning(f"index {index} is less than length {length}. return length from index 0. Please assgin start_index.")
                index = length
        else:
        ...
        if length > 1:
            ...
            if self._step_index >= length - 1:
                ...
                if self._auto_reset:
                    ...
                    logger.warning(f"not more data on index {self._step_index}")
                return pd.DataFrame()
        else:
    ...
    def reset(self, mode: str = None, retry=0) -> bool:
        ...
        if mode is not None:
            if retry <= 10:
                ...
                logger.warning("Data client reset with {mode} mode retried over 10. index was not correctly reset.")
                return False
        # raise index change event
        return True
...
class CSVChunkClient(CSVClientBase):
    ...
    def _read_csv(self, files, symbols=[], columns=[], date_col=None, skiprows=None, start_date=None, frame=None):
        ...
        if len(duplicated_key_set) > 0:
            logger.warning("same file is initialized with chunkmode. TextReader is ignored.")
    ...
    def __read_chunk_data(self, symbol):
        if symbol in self.TRS:
            ...
            try:
                ...
                logger.error(f"failed read {symbol} file by {e}")
        else:
            logger.debug(f"{symbol} is not in TRS")
        return pd.DataFrame()
