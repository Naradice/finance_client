...
class TestDataset:
    ...
    def _get_output(self, idx):
        if type(idx) is slice:
            ...
            out_idx = idx + self.observation_length
...
class TestCSVClient(unittest.TestCase):
    ...
    def test_single_dataset(self):
        length = 30
        client = CSVClient(
            files=csv_file, observation_length=length, logger=logger, date_column=datetime_column, start_index=length
        )
        single_dataset = TestDataset(client, length)
        batch_size = 16
