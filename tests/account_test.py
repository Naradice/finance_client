...
class ManagerTest(unittest.TestCase):
    ...
    def test_get_daily_realized_pnl_tzinfo(self):
        ...
        # index which is definitely tomorrow in JST but today in UTC, to test if tz_info is correctly handled
        tomorrow_index = today_index + pd.Timedelta(days=1)
        delta_to_close = pd.Timedelta(minutes=1)
        position2 = manager.open_position(POSITION_SIDE.short, "test", price=200, volume=2, index=tomorrow_index)
        manager.close_position(position1.id, price=110, volume=1, index=today_index + delta_to_close)
        manager.close_position(position2.id, price=190, volume=2, index=tomorrow_index + delta_to_close)
        daily_realized_pnl = manager.get_daily_realized_pnl()
        expected_realized_pnl = (110 - 100) * position1.trade_unit * position1.leverage * position1.volume
        self.assertEqual(daily_realized_pnl, expected_realized_pnl)
    ...
    def test_get_daily_realized_pnl_with_mismatched_tzinfo(self):
        ...
        position1 = manager.open_position(POSITION_SIDE.long, "test", price=100, volume=1, index=yesterday_index)
        manager.close_position(position1.id, price=110, volume=1, index=yesterday_index + pd.Timedelta(minutes=1))
        # since +9 is specified in manager's tz_info, the closed position should not be considered as closed today, so it should be included in daily realized pnl
        self.assertEqual(manager.get_daily_realized_pnl(), 0)

        today_index = pd.Timestamp(year=now.year, month=now.month, day=now.day, hour=20, minute=0, second=0, tz=client_tz_info) - pd.Timedelta(days=1)
        position2 = manager.open_position(POSITION_SIDE.long, "test", price=100, volume=1, index=today_index)
    ...
    # test daily realized pnl with SQL storage
    def test_get_daily_realized_pnl_with_sql_storage(self):
    ...
    def test_get_daily_realized_pnl_with_sql_storage_and_tzinfo(self):
        if os.path.exists("./unit_test.db"):
            os.remove("./unit_test.db")
    ...
    def test_get_daily_realized_pnl_with_sql_storage_and_mismatched_tzinfo(self):
        if os.path.exists("./unit_test.db"):
            os.remove("./unit_test.db")
        key = "account_test_pnl_sql_mismatched_tz"
        storage = db.PositionSQLiteStorage("./unit_test.db", key, username=key)
        log_storage = db.LogSQLiteStorage("./unit_test.db", key, username=key)
        client_tz_info = datetime.timezone(datetime.timedelta(hours=0))  # UTC
        manager = Manager(10000, position_storage=storage, log_storage=log_storage, tz_info=datetime.timezone(datetime.timedelta(hours=9)))
