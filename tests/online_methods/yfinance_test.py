...
class TestYFClient(unittest.TestCase):
    ...
    def test_get_rates(self):
        length = 10
        rates = self.client.get_ohlc(length=length)
    ...
    def test_get_rates_with_indicater(self):
        from finance_client.fprocess.fprocess.idcprocess import (
            BBANDProcess, MACDProcess, RangeTrendProcess)
        ...
        data = self.client.get_ohlc(length=100, idc_processes=[macd_p, bband_process, rtp_p])
