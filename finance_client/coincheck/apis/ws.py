...
class TradeHistory:
    ...
    def subscribe(self, on_tick: Callable, pair="btc_jpy"):
        ...
        while self.ready_state != 1:
            time.sleep(1)
        self.ws.send(json.dumps({"type": "subscribe", "channel": f"{pair}-trades"}))
...
class Orders:
    ...
    def __init__(self, debug=False) -> None:
        ...
        websocket.enableTrace(debug)
        self.ws = websocket.WebSocketApp(
            f"{self.baseUrl}", on_open=self.__on_open, on_message=self.__on_message, on_error=self.__on_error, on_close=self.__on_close
        )
        self.ready_state = 0  # connecting
    ...
    def subscribe(self, pair="btc_jpy"):
        while self.ready_state != 1:
            time.sleep(1)
        self.ws.send(json.dumps({"type": "subscribe", "channel": f"{pair}-orderbook"}))
