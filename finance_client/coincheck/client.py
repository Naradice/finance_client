...
class CoinCheckClient(Client):
    ...
    def __store_ticks(self, tick):
        ...
        if self.frame_ohlcv is None:
            ...
            self.frame_ohlcv = pd.DataFrame({"open": tick_price, "high": tick_price, "low": tick_price, "close": tick_price, "volume": tick_volume}, index=[self.current_frame])
    ...
    def __init__(self, ACCESS_ID=None, ACCESS_SECRET=None, initialized_with = None, return_intermidiate_data=False, simulation=False, budget=1000000, indicater_processes: list = [], post_processes: list = [], frame: int = 30, do_render=False, logger=None):
        ...
        if self.frame < Frame.H1:
            ...
            delta = datetime.timedelta(hours=additional_hours, minutes=current_time.minute, seconds=current_time.second, microseconds=current_time.microsecond)
            self.current_frame = current_time - delta
            ...
            delta = datetime.timedelta(days=additional_days, hours=current_time.hour, minutes=current_time.minute, seconds=current_time.second, microseconds=current_time.microsecond)
            self.current_frame = current_time - delta
            ...
            if last_frame_time is not None:
                ...
                ## When last_frame_time is less than current_Time, if difference is just a frame ignore it otherwise assuming server time and client time are mismatching. so convert(reduce) server datetime
                    ...
                    if diff_mins != self.frame:
                        self.logger.warn(f"Time difference between initialize client and coincheck server is {diff_mins} mins. try to fit it to local time. If you feel this is strange, please stop this script.")
                        fitting_time = delta - datetime.timedelta(minutes=self.frame)#Not sure last tick is actually on time. So put it on 1 frame before
