...
class CSVClient(Client):
    ...
    def __initialize_file_name_func(self, file_names:list):
        ...
        if len(same_strs) > 0:
            if len(same_strs) > 1:
                ...
                prefix = same_strs[0]
                suffix = same_strs[-1]
                self.__get_symbol_from_filename = lambda file_name: file_name.replace(prefix, "").replace(suffix, "")
                ...
                suffix = same_strs[-1]
                self.__get_symbol_from_filename = lambda file_name: file_name.replace(suffix, "")
                if self.file_name_generator is None:
    ...
    def __initialize_date_index(self, data:pd.DataFrame, ascending:bool, start_date:datetime.datetime=None):
    ...
    def __read_csv__(self, files, columns=[], date_col=None, skiprows=None, start_date=None, chunksize=None, ascending=True):
        ...
        if date_col is not None:
            ...
            if time_column in __columns:
                ...
                is_date_index = True
        if is_date_index:
            self.__initialize_date_index(data, ascending, start_date)
    ...
    def __init__(self, files:list = None, columns = [], date_column = None, 
        ...
        if files is not None:
            ...
            self.data = self.__read_csv__(files, columns, date_column, skiprows, start_date, chunksize)
            if out_frame != None:
